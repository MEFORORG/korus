...
| `scripts/wiki/query.ps1` | Searches the inbox, and the record repository's log with `-RecordRepo`. Prints `no note` below the match floor. Exits 0, saying so, on an unreachable store. Warns on stdout when an unreadable file may hide a retirement. Logs queries locally | [Fleet wiki spec](../specs/002-fleet-wiki/spec.md) |
