...
| `scripts/wiki/query.ps1` | Searches the inbox, and the record repository's log with `-RecordRepo`. Prints `no note` below the match floor. Says so when a store is unreachable, and still exits 0. Warns on stdout when an unreadable file may hide a retirement. Hides the import's merge records unless `-History`. Appends one line per query to a local log under the state root | [Fleet wiki spec](../specs/002-fleet-wiki/spec.md) |
