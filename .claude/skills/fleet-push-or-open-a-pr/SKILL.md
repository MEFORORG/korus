...
| **Read again 2026-09-24, at `MEFORORG/korus`** | korus moved from `wshallwshall`, and the old slug redirects. `mergeQueue(branch:"main")` still returns `null` there, and the engine control still returns `MQ_kwDOS5JJRs4AA9_8`. The row above keeps the slug it was read at. |
