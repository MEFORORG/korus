...
| **The slug in that row is stale, read 2026-09-24** | The row keeps what it measured. Today the engine's `private` and the vault's `origin` both read `MEFORORG/MessageFoundry-vault` with `git remote get-url`. `wshallwshall/MessageFoundry` redirects there. They are still one repository. |
