...
| Who OPENS -- NARROWED 2026-09-18 | That row read *"and opens its own PR"*. It still holds for every seat except a **Builder working to a Manager's brief**: that Builder pushes and reports, and **the Manager opens the pull request**. Since 2026-09-23 that means a Manager's SUBAGENT only: a session started from a chip opens its own, and you may receive its handover directly. [MANAGER.md](MANAGER.md), *A session started from a chip opens its own pull request*. |
